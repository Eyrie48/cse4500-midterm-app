...
<form method="post" action="{{ route('manufacturers.update', ['manufacturer'=>$manufacturer->id]) }}" >
    @csrf
    @method('PUT')
    <x-adminlte-input name="sales_name" value="{{ $manufacturer->sales_name }}" label="Sales Name" />
    <x-adminlte-input name="sales_email" value="{{ $manufacturer->sales_email }}" label="Sales Email" />
    <x-adminlte-input name="tech_name" value="{{ $manufacturer->tech_name }}" label="Tech Name" />
